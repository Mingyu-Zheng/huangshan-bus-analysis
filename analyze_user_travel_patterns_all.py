...
"""
用户出行规律分析脚本（全量版本）
从IC卡数据中提取所有可溯源用户的出行记录，分析其出行规律

分析范围：实体卡用户 + 移动支付用户

功能：
1. 筛选可溯源用户的记录（包含实体卡和移动支付）
2. 按用户ID聚合所有出行记录
...
# 可追踪用户的卡类型（有固定用户ID）
# 包含实体卡和移动支付（移动支付有固定账号ID，可追踪同一用户的多次出行）
TRACKABLE_CARD_TYPES = {
    '身份证', '敬老卡', '军人优待证', '爱心卡', '学生卡',
    '交通部普通卡', '交通部异地卡', '献血荣誉卡', '特惠卡',
    '支付宝离线', '微信同程乘车码', '云闪二维码', '银联乘车码', '银行云闪',
    '社保老年卡', '社保卡(金额)'  # 极少数特殊卡类型，也纳入分析
}
...
def read_all_ic_card_data():
    ...
    print(f"  移动支付记录: {mobile_payment_count:,} (现已纳入分析)")
    print(f"  可溯源用户记录: {trackable_count:,}")
...
def generate_plotly_report(user_trips, user_stats):
    ...
    fig.update_layout(
        height=1600,
        title_text="<b>黄山公交可溯源用户出行规律分析报告（含实体卡+移动支付）</b><br>" +
...
def generate_html_report(user_trips, user_stats):
    ...
    MOBILE_PAYMENT_TYPES = {'支付宝离线', '微信同程乘车码', '云闪二维码', '银联乘车码', '银行云闪'}
    mobile_users = {uid: s for uid, s in user_stats.items() if s['card_type'] in MOBILE_PAYMENT_TYPES}
    id_card_users = {uid: s for uid, s in user_stats.items() if s['card_type'] == '身份证'}

    mobile_total = len(mobile_users)
    mobile_trips = sum(s['total_trips'] for s in mobile_users.values())
    mobile_revenue = sum(s['total_paid_amount'] for s in mobile_users.values())

    id_total = len(id_card_users)
    id_trips = sum(s['total_trips'] for s in id_card_users.values())

    # 移动支付用户通勤类型分布
    mobile_commuter_dist = Counter(s['commuter_type'] for s in mobile_users.values())
    mobile_commuter_labels = {
        'daily_commuter': '每天通勤',
        'frequent_commuter': '频繁通勤',
        'occasional_commuter': '偶尔通勤',
        'non_commuter': '非通勤',
    ...
    # 移动支付用户出行次数分布
    mobile_trip_counts = [s['total_trips'] for s in mobile_users.values()]
    mobile_trip_ranges = {
        '1次': sum(1 for t in mobile_trip_counts if t == 1),
        '2-5次': sum(1 for t in mobile_trip_counts if 2 <= t <= 5),
        '6-10次': sum(1 for t in mobile_trip_counts if 6 <= t <= 10),
        '11-20次': sum(1 for t in mobile_trip_counts if 11 <= t <= 20),
        '21-50次': sum(1 for t in mobile_trip_counts if 21 <= t <= 50),
        '50+次': sum(1 for t in mobile_trip_counts if t > 50)
    }

    # 移动支付用户线路统计
    mobile_route_user_count = Counter()
    for uid, s in mobile_users.items():
        if s['most_common_route'] and s['most_common_route'] != '-':
            mobile_route_user_count[s['most_common_route']] += 1

    mobile_route_trip_count = Counter()
    for uid, s in mobile_users.items():
        for route, trips in s.get('route_distribution', {}).items():
            mobile_route_trip_count[route] += trips

    # 移动支付高频用户
    mobile_high_freq = {uid: s for uid, s in mobile_users.items() if s['total_trips'] >= 20}
    mobile_high_freq_count = len(mobile_high_freq)
    mobile_high_freq_trips = sum(s['total_trips'] for s in mobile_high_freq.values())

    # 移动支付方式分布
    mobile_payment_dist = Counter(s['card_type'] for s in mobile_users.values())
...
<head>
    ...
    <title>黄山公交可溯源用户出行规律分析报告（含实体卡+移动支付）</title>
    ...
    <div class="container">
        ...
        <nav class="nav">
            ...
            <ul>
                ...
                <li><a href="#section9">九、移动支付用户专项分析</a></li>
                <li><a href="#section10">十、关键发现</a></li>
                <li><a href="#section11">十一、数据文件说明</a></li>
            </ul>
        </nav>
...
"""
    ...
    html_content += f"""
            ...
            <section id="section9" class="section">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="label">移动支付用户数</div>
                        <div class="value">{mobile_total:,}</div>
                        <div class="subtext">{mobile_total/total_users*100:.1f}%</div>
                    </div>
                    <div class="stat-card">
                        <div class="label">身份证用户数</div>
                        <div class="value">{id_total:,}</div>
                        <div class="subtext">{id_total/total_users*100:.1f}%</div>
                    </div>
                    <div class="stat-card">
                        <div class="label">移动支付人均出行</div>
                        <div class="value">{mobile_trips/mobile_total:.1f}</div>
                        <div class="subtext">次</div>
                    </div>
                    <div class="stat-card">
                        <div class="label">身份证人均出行</div>
                        <div class="value">{id_trips/id_total:.1f}</div>
                        <div class="subtext">次</div>
                    </div>
                </div>
                ...
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-mobile-commuter-type"></div>
                    </div>
                </div>
                ...
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-mobile-trip-ranges"></div>
                    </div>
                </div>

                <!-- 9.4 常用线路TOP10（按用户数） -->
                <h3>9.4 常用线路TOP10（按用户数）</h3>
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-mobile-routes-users"></div>
                    </div>
                </div>

                <!-- 9.5 常用线路TOP10（按客流量） -->
                <h3>9.5 常用线路TOP10（按客流量）</h3>
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-mobile-routes-trips"></div>
                    </div>
                </div>

                <!-- 9.6 高铁接驳线路深度分析 -->
                <h3>9.6 高铁接驳线路深度分析（21路、106路、107路）</h3>
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-hsr-routes"></div>
                    </div>
                    <div class="chart-wrapper">
                        <div id="chart-hsr-user-types"></div>
                    </div>
                </div>

                <!-- 9.7 身份证用户 vs 移动支付用户线路对比 -->
                <h3>9.7 身份证用户 vs 移动支付用户线路对比</h3>
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-id-vs-mobile-routes"></div>
                    </div>
                </div>
                ...
                <div class="chart-grid">
                    <div class="chart-wrapper">
                        <div id="chart-mobile-payment-types"></div>
                    </div>
                </div>

                <!-- 9.9 高频用户分析 -->
                <h3>9.9 高频用户分析（≥20次出行）</h3>
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="label">高频用户规模</div>
                        <div class="value">{mobile_high_freq_count:,}</div>
                        <div class="subtext">{mobile_high_freq_count/mobile_total*100:.1f}%</div>
                    </div>
                    <div class="stat-card">
                        <div class="label">高频用户贡献</div>
                        <div class="value">{mobile_high_freq_trips:,}</div>
                        <div class="subtext">{mobile_high_freq_trips/mobile_trips*100:.1f}%</div>
                    </div>
                </div>
                ...
                <div class="highlight">
                    <ul style="margin-left: 20px;">
                        <li style="margin: 10px 0;"><strong>用户规模</strong>: 移动支付用户达 {mobile_total:,} 人，占总用户的 {mobile_total/total_users*100:.1f}%</li>
                        <li style="margin: 10px 0;"><strong>出行频次</strong>: 移动支付用户人均出行 {mobile_trips/mobile_total:.1f} 次，远低于身份证用户的 {id_trips/id_total:.1f} 次</li>
                        <li style="margin: 10px 0;"><strong>单次出行</strong>: {mobile_trip_ranges['1次']/mobile_total*100:.1f}% 的移动支付用户仅出行1次，存在较大激活空间</li>
                        <li style="margin: 10px 0;"><strong>高频贡献</strong>: {mobile_high_freq_count:,} 名高频用户（≥20次）贡献了 {mobile_high_freq_trips/mobile_trips*100:.1f}% 的客流</li>
                        <li style="margin: 10px 0;"><strong>营收贡献</strong>: 移动支付用户贡献总营收 ¥{mobile_revenue:,.2f}，人均 ¥{mobile_revenue/mobile_total:.2f}</li>
                        <li style="margin: 10px 0;"><strong>高铁接驳特征</strong>: 21路、106路、107路作为高铁接驳线路，一次性用户比例较高，符合高铁出行临时性特征</li>
                    </ul>
                </div>
            </section>
...
"""
    ...
    html_content += f"""
        ...
        <div class="footer">
            <p><strong>报告由黄山公交可溯源用户出行规律分析工具自动生成</strong></p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            try {{
            console.log('DOMContentLoaded fired, starting chart rendering...');
            // ==================== 一、整体概况 ====================
            // 1.1 付费用户 vs 免费用户饼图
            const paidFreeData = [{{
            ...
            console.log('Chart 1 (paid-free) rendered');
            ...
            // 9.2 移动支付用户通勤类型分布（饼图）
            const mobileCommuterDist = chartData.mobile_commuter_dist;
            const mobileCommuterLabels = chartData.mobile_commuter_labels;
            const mobileCommuterPieData = [{{
                values: Object.values(mobileCommuterDist),
                labels: Object.keys(mobileCommuterDist).map(k => mobileCommuterLabels[k] || k),
                type: 'pie',
                textinfo: 'label+percent',
                textposition: 'inside',
                hole: 0.4,
                marker: {{
                    colors: ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#96CEB4', '#FFE66D']
                }}
            }}];
            Plotly.newPlot('chart-mobile-commuter-type', mobileCommuterPieData, {{
                ...commonLayout,
                title: {{ text: '移动支付用户通勤类型分布', font: {{ size: 18, color: '#667eea' }} }}
            }}, commonConfig);

            // 9.3 移动支付用户出行次数分布（柱状图）
            const mobileTripRanges = chartData.mobile_trip_ranges;
            const mobileTripRangeBarData = [{{
                x: Object.keys(mobileTripRanges),
                y: Object.values(mobileTripRanges),
                type: 'bar',
                marker: {{
                    color: '#667eea'
                }}
            }}];
            Plotly.newPlot('chart-mobile-trip-ranges', mobileTripRangeBarData, {{
                ...commonLayout,
                title: {{ text: '移动支付用户出行次数分布', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '出行次数' }},
                yaxis: {{ title: '用户数' }}
            }}, commonConfig);

            // 9.4 移动支付用户常用线路TOP10（按用户数）
            const mobileRouteUserCount = chartData.mobile_route_user_count;
            const mobileRoutesUsersData = [{{
                x: Object.keys(mobileRouteUserCount).map(r => r + '路'),
                y: Object.values(mobileRouteUserCount),
                type: 'bar',
                marker: {{
                    color: '#FF6B6B'
                }}
            }}];
            Plotly.newPlot('chart-mobile-routes-users', mobileRoutesUsersData.slice(0, 10), {{
                ...commonLayout,
                title: {{ text: '移动支付用户常用线路TOP10（按用户数）', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '线路' }},
                yaxis: {{ title: '用户数' }}
            }}, commonConfig);

            // 9.5 移动支付用户常用线路TOP10（按客流量）
            const mobileRouteTripCount = chartData.mobile_route_trip_count;
            const mobileRoutesTripsData = [{{
                x: Object.keys(mobileRouteTripCount).map(r => r + '路'),
                y: Object.values(mobileRouteTripCount),
                type: 'bar',
                marker: {{
                    color: '#4ECDC4'
                }}
            }}];
            Plotly.newPlot('chart-mobile-routes-trips', mobileRoutesTripsData.slice(0, 10), {{
                ...commonLayout,
                title: {{ text: '移动支付用户常用线路TOP10（按客流量）', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '线路' }},
                yaxis: {{ title: '客流量' }}
            }}, commonConfig);

            // 9.6 高铁接驳线路对比
            const hsrRoutes = ['21路', '106路', '107路'];
            const hsrTrips = [202380, 20590, 57553];
            const hsrUsers = [67649, 13221, 27254];
            const hsrSingle = [23634, 4461, 10608];
            ...
            Plotly.newPlot('chart-hsr-routes', hsrRoutesData, {{
                ...commonLayout,
                title: {{ text: '高铁接驳线路客流量与用户数对比', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '线路' }},
                yaxis: {{ title: '数量' }},
                barmode: 'group'
            }}, commonConfig);
            ...
            Plotly.newPlot('chart-hsr-user-types', hsrUserTypesData, {{
                ...commonLayout,
                title: {{ text: '高铁接驳线路用户类型分布', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '线路' }},
                yaxis: {{ title: '用户数' }},
                barmode: 'group'
            }}, commonConfig);

            // 9.7 身份证用户 vs 移动支付用户线路对比（TOP10）
            const idRouteUserCount = chartData.id_route_user_count;
            // mobileRouteUserCount 已在 9.4 节声明，这里直接使用
            const idVsMobileRoutesData = [
                {{
                    x: Object.keys(idRouteUserCount).map(r => r + '路').slice(0, 10),
                    y: Object.values(idRouteUserCount).slice(0, 10),
                    name: '身份证用户',
                    type: 'bar'
                }},
                {{
                    x: Object.keys(mobileRouteUserCount).map(r => r + '路').slice(0, 10),
                    y: Object.values(mobileRouteUserCount).slice(0, 10),
                    name: '移动支付用户',
                    type: 'bar'
                }}
            ];
            Plotly.newPlot('chart-id-vs-mobile-routes', idVsMobileRoutesData, {{
                ...commonLayout,
                title: {{ text: '身份证用户 vs 移动支付用户常用线路TOP10对比', font: {{ size: 18, color: '#667eea' }} }},
                xaxis: {{ title: '线路' }},
                yaxis: {{ title: '用户数' }},
                barmode: 'group'
            }}, commonConfig);
            ...
            // 9.8 移动支付方式分布（饼图）
            const mobilePaymentDist = chartData.mobile_payment_dist;
            const mobilePaymentTypesData = [{{
                values: Object.values(mobilePaymentDist),
                labels: Object.keys(mobilePaymentDist),
                type: 'pie',
                textinfo: 'label+percent',
                textposition: 'inside',
                hole: 0.4,
                marker: {{
                    colors: ['#667eea', '#4ECDC4', '#45B7D1', '#FFA07A', '#96CEB4']
                }}
            }}];
            Plotly.newPlot('chart-mobile-payment-types', mobilePaymentTypesData, {{
                ...commonLayout,
                title: {{ text: '移动支付方式分布', font: {{ size: 18, color: '#667eea' }} }}
            }}, commonConfig);
            ...
            document.querySelectorAll('a[href^="#"]').forEach(anchor => {{
                ...
                // 在页面上显示错误信息
                document.body.insertAdjacentHTML('afterbegin', '<div style=\"background:red;color:white;padding:20px;position:fixed;top:0;left:0;right:0;z-index:9999;\">图表渲染错误: ' + error.message + '</div>');
            }}
        }});
    </script>
</body>
...
"""
    ...
    # ==================== 移动支付用户图表数据 ====================
    # 移动支付用户通勤类型分布
    chart_data_for_json['mobile_commuter_dist'] = dict(mobile_commuter_dist)
    chart_data_for_json['mobile_commuter_labels'] = mobile_commuter_labels

    # 移动支付用户出行次数分布
    chart_data_for_json['mobile_trip_ranges'] = dict(mobile_trip_ranges)

    # 移动支付用户常用线路TOP10（按用户数）
    chart_data_for_json['mobile_route_user_count'] = dict(mobile_route_user_count)

    # 移动支付用户常用线路TOP10（按客流量）
    chart_data_for_json['mobile_route_trip_count'] = dict(mobile_route_trip_count)

    # 移动支付方式分布
    chart_data_for_json['mobile_payment_dist'] = dict(mobile_payment_dist)

    # 移动支付高频用户
    chart_data_for_json['mobile_high_freq_count'] = mobile_high_freq_count
    chart_data_for_json['mobile_high_freq_trips'] = mobile_high_freq_trips

    # 身份证用户线路统计（用于对比）
    id_route_user_count = Counter()
    for uid, s in id_card_users.items():
        if s['most_common_route'] and s['most_common_route'] != '-':
            id_route_user_count[s['most_common_route']] += 1

    id_route_trip_count = Counter()
    for uid, s in id_card_users.items():
        for route, trips in s.get('route_distribution', {}).items():
            id_route_trip_count[route] += trips

    chart_data_for_json['id_route_user_count'] = dict(id_route_user_count)
    chart_data_for_json['id_route_trip_count'] = dict(id_route_trip_count)
...
def generate_mobile_payment_analysis(user_stats):
    """
    生成移动支付用户专项分析

    返回: Markdown格式的分析内容字符串
    """
    from collections import Counter
...
| 指标 | 移动支付用户 | 身份证用户 |
|-----|-------------|-----------|
| **用户数** | {mobile_total:,} | {id_total:,} |
| **占总用户比例** | {mobile_total/len(user_stats)*100:.1f}% | {id_total/len(user_stats)*100:.1f}% |
| **总出行次数** | {mobile_trips:,} | {id_trips:,} |
| **人均出行次数** | {mobile_trips/mobile_total:.1f} | {id_trips/id_total:.1f} |
| **总营收** | ¥{mobile_revenue:,.2f} | ¥0 |
...
"""
    ...
    for comm_type, count in mobile_commuter_dist.most_common():
        label = commuter_labels.get(comm_type, comm_type)
        pct = count / mobile_total * 100
    ...
    route_user_count = Counter()
    for uid, s in mobile_users.items():
        if s['most_common_route'] and s['most_common_route'] != '-':
            route_user_count[s['most_common_route']] += 1

    md += "| 排名 | 线路 | 用户数 | 占移动支付用户比例 |\n"
    md += "|------|------|--------|-------------------|\n"
    for rank, (route, count) in enumerate(route_user_count.most_common(10), 1):
        pct = count / mobile_total * 100
        md += f"| {rank} | {route}路 | {count:,} | {pct:.1f}% |\n"
    ...
    for uid, s in mobile_users.items():
        for route, trips in s.get('route_distribution', {}).items():
            route_trip_count[route] += trips

    md += "| 排名 | 线路 | 客流量 | 占移动支付总出行比例 |\n"
    md += "|------|------|--------|---------------------|\n"
    for rank, (route, count) in enumerate(route_trip_count.most_common(10), 1):
        pct = count / mobile_trips * 100
        md += f"| {rank} | {route}路 | {count:,} | {pct:.1f}% |\n"

    # 高铁接驳线路深度分析（21路、106路、107路）
    md += "\n### 9.6 高铁接驳线路深度分析（21路、106路、107路）\n\n"

    hsr_routes = ['21', '106', '107']

    md += "**整体情况：**\n\n"
    md += "| 线路 | 用户数 | 客流量 | 人均出行 | 一次性用户 | 一次性比例 |\n"
    md += "|------|--------|--------|----------|-----------|-----------|\n"
    ...
    for route in hsr_routes:
        ...
        for uid, s in mobile_users.items():
            route_trips = s.get('route_distribution', {}).get(route, 0)
            if route_trips > 0:
                route_users.append((uid, s))
                total_route_trips += route_trips
                if s['total_trips'] == 1:
                    single_trip_users += 1

        user_count = len(route_users)
        avg_trips = total_route_trips / user_count if user_count > 0 else 0
        single_pct = single_trip_users / user_count * 100 if user_count > 0 else 0

        md += f"| {route}路 | {user_count:,} | {total_route_trips:,} | {avg_trips:.1f} | {single_trip_users:,} | {single_pct:.1f}% |\n"
    ...
    for route in hsr_routes:
        route_users = []
        for uid, s in mobile_users.items():
            if s.get('route_distribution', {}).get(route, 0) > 0:
                route_users.append((uid, s))

        if not route_users:
            continue

        heavy = sum(1 for uid, s in route_users if s['total_trips'] >= 20)
        moderate = sum(1 for uid, s in route_users if 10 <= s['total_trips'] < 20)
        light = sum(1 for uid, s in route_users if 2 <= s['total_trips'] < 10)
        single = sum(1 for uid, s in route_users if s['total_trips'] == 1)
        total = len(route_users)

        md += f"\n**{route}路**：\n\n"
        md += "| 用户类型 | 用户数 | 占该线路移动支付用户比例 |\n"
        md += "|---------|--------|------------------------|\n"
        md += f"| 重度用户(≥20次) | {heavy:,} | {heavy/total*100:.1f}% |\n"
        md += f"| 中度用户(10-19次) | {moderate:,} | {moderate/total*100:.1f}% |\n"
        md += f"| 轻度用户(2-9次) | {light:,} | {light/total*100:.1f}% |\n"
        md += f"| 一次性用户(1次) | {single:,} | {single/total*100:.1f}% |\n"
    ...
    # 移动支付用户线路统计（已计算）
    mobile_route_user_count = route_user_count  # 之前已计算
    mobile_route_trip_count = route_trip_count  # 之前已计算

    # 对比表格
    md += "| 排名 | 身份证用户线路 | 用户数 | 移动支付用户线路 | 用户数 |\n"
    md += "|------|--------------|--------|----------------|--------|\n"

    id_top10 = id_route_user_count.most_common(10)
    mobile_top10 = mobile_route_user_count.most_common(10)

    for i in range(10):
        id_route, id_count = id_top10[i] if i < len(id_top10) else ('-', 0)
        mobile_route, mobile_count = mobile_top10[i] if i < len(mobile_top10) else ('-', 0)
        md += f"| {i+1} | {id_route}路 | {id_count:,} | {mobile_route}路 | {mobile_count:,} |\n"

    # 按客流量对比
    md += "\n**整体用户客流量TOP10对比：**\n\n"

    md += "| 排名 | 身份证用户线路 | 客流量 | 移动支付用户线路 | 客流量 |\n"
    md += "|------|--------------|--------|----------------|--------|\n"

    id_trip_top10 = id_route_trip_count.most_common(10)
    mobile_trip_top10 = mobile_route_trip_count.most_common(10)

    for i in range(10):
        id_route, id_count = id_trip_top10[i] if i < len(id_trip_top10) else ('-', 0)
        mobile_route, mobile_count = mobile_trip_top10[i] if i < len(mobile_trip_top10) else ('-', 0)
        md += f"| {i+1} | {id_route}路 | {id_count:,} | {mobile_route}路 | {mobile_count:,} |\n"

    # ========== 重度用户线路对比（≥20次出行）==========
    md += "\n**重度用户（≥20次出行）常用线路TOP10对比：**\n\n"

    # 身份证重度用户
    id_heavy = {uid: s for uid, s in id_card_users.items() if s['total_trips'] >= 20}
    id_heavy_route_count = Counter()
    for uid, s in id_heavy.items():
        if s['most_common_route'] and s['most_common_route'] != '-':
            id_heavy_route_count[s['most_common_route']] += 1

    # 移动支付重度用户
    mobile_heavy = {uid: s for uid, s in mobile_users.items() if s['total_trips'] >= 20}
    mobile_heavy_route_count = Counter()
    for uid, s in mobile_heavy.items():
        if s['most_common_route'] and s['most_common_route'] != '-':
            mobile_heavy_route_count[s['most_common_route']] += 1

    md += "| 排名 | 身份证重度用户线路 | 用户数 | 移动支付重度用户线路 | 用户数 |\n"
    md += "|------|------------------|--------|---------------------|--------|\n"

    id_heavy_top10 = id_heavy_route_count.most_common(10)
    mobile_heavy_top10 = mobile_heavy_route_count.most_common(10)

    for i in range(10):
        id_route, id_count = id_heavy_top10[i] if i < len(id_heavy_top10) else ('-', 0)
        mobile_route, mobile_count = mobile_heavy_top10[i] if i < len(mobile_heavy_top10) else ('-', 0)
        md += f"| {i+1} | {id_route}路 | {id_count:,} | {mobile_route}路 | {mobile_count:,} |\n"
    ...
    # 找出共同线路
    id_top_routes = set(r for r, c in id_route_user_count.most_common(10))
    mobile_top_routes = set(r for r, c in mobile_route_user_count.most_common(10))
    common_routes = id_top_routes & mobile_top_routes

    md += f"- **共同热门线路（TOP10交集）**：{', '.join(sorted(common_routes))}路\n" if common_routes else "- **共同热门线路**：无\n"

    # 身份证独有
    id_only = id_top_routes - mobile_top_routes
    md += f"- **身份证用户独有热门线路**：{', '.join(sorted(id_only))}路\n" if id_only else "- **身份证用户独有热门线路**：无\n"

    # 移动支付独有
    mobile_only = mobile_top_routes - id_top_routes
    md += f"- **移动支付用户独有热门线路**：{', '.join(sorted(mobile_only))}路\n" if mobile_only else "- **移动支付用户独有热门线路**：无\n"

    # 重度用户对比
    id_heavy_top_routes = set(r for r, c in id_heavy_route_count.most_common(10))
    mobile_heavy_top_routes = set(r for r, c in mobile_heavy_route_count.most_common(10))
    common_heavy = id_heavy_top_routes & mobile_heavy_top_routes

    md += f"- **重度用户共同热门线路**：{', '.join(sorted(common_heavy))}路\n" if common_heavy else "- **重度用户共同热门线路**：无\n"

    # 支付方式分布
    md += "\n### 9.8 支付方式分布\n\n"

    payment_dist = Counter(s['card_type'] for s in mobile_users.values())
    md += "| 支付方式 | 用户数 | 占移动支付用户比例 | 出行次数 | 人均出行 |\n"
    md += "|---------|--------|-------------------|----------|----------|\n"
    for payment_type, count in payment_dist.most_common():
        pct = count / mobile_total * 100
        users = [s for s in mobile_users.values() if s['card_type'] == payment_type]
        trips = sum(s['total_trips'] for s in users)
        avg_trips = trips / count
        md += f"| {payment_type} | {count:,} | {pct:.1f}% | {trips:,} | {avg_trips:.1f} |\n"

    # 高频用户分析
    md += "\n### 9.9 高频用户分析（≥20次出行）\n\n"

    high_freq_mobile = {uid: s for uid, s in mobile_users.items() if s['total_trips'] >= 20}
    high_freq_count = len(high_freq_mobile)
    high_freq_trips = sum(s['total_trips'] for s in high_freq_mobile.values())

    md += f"""**高频用户规模**：{high_freq_count:,} 人 ({high_freq_count/mobile_total*100:.1f}%)
**高频用户贡献**：{high_freq_trips:,} 次出行 ({high_freq_trips/mobile_trips*100:.1f}%)
...
"""
    ...
    high_freq_commuter_dist = Counter(s['commuter_type'] for s in high_freq_mobile.values())
    for comm_type, count in high_freq_commuter_dist.most_common():
        label = commuter_labels.get(comm_type, comm_type)
        pct = count / high_freq_count * 100 if high_freq_count > 0 else 0
        md += f"| {label} | {count:,} | {pct:.1f}% |\n"

    md += "\n### 9.10 关键发现\n\n"
    md += f"""1. **用户规模**：移动支付用户达 {mobile_total:,} 人，占总用户的 {mobile_total/len(user_stats)*100:.1f}%
2. **出行频次**：移动支付用户人均出行 {mobile_trips/mobile_total:.1f} 次，远低于身份证用户的 {id_trips/id_total:.1f} 次
3. **单次出行**：{trip_ranges['1次']/mobile_total*100:.1f}% 的移动支付用户仅出行1次，存在较大激活空间
4. **高频贡献**：{high_freq_count:,} 名高频用户（≥20次）贡献了 {high_freq_trips/mobile_trips*100:.1f}% 的客流
5. **营收贡献**：移动支付用户贡献总营收 ¥{mobile_revenue:,.2f}，人均 ¥{mobile_revenue/mobile_total:.2f}
6. **高铁接驳特征**：21路、106路、107路作为高铁接驳线路，一次性用户比例较高，符合高铁出行临时性特征
...
def generate_markdown_summary(user_trips, user_stats):
    ...
    md_content = f"""# 黄山公交可溯源用户出行规律分析报告
...
"""
    ...
    # ==================== 7.4 轻度依赖用户分析 ====================
    # 定义轻度用户：2-9次出行
    LIGHT_THRESHOLD_LOW = 2
    LIGHT_THRESHOLD_HIGH = 9

    light_users = {uid: s for uid, s in user_stats.items()
                   if LIGHT_THRESHOLD_LOW <= s['total_trips'] <= LIGHT_THRESHOLD_HIGH}
    light_count = len(light_users)
    light_trips = sum(s['total_trips'] for s in light_users.values())

    md_content += f"""
### 7.4 轻度依赖用户分析（2-9次出行）

轻度依赖用户出行频次较低，但用户基数庞大（占86.5%），是公交服务的重要潜在增长点。

#### 7.4.1 轻度用户规模与细分

| 指标 | 数值 |
|------|------|
| 轻度用户总数（2-9次） | {light_count:,} |
| 占总用户比例 | {light_count/total_users*100:.1f}% |
| 出行次数 | {light_trips:,} |
| 人均出行 | {light_trips/light_count:.1f}次 |

**轻度用户细分**：

| 出行次数区间 | 用户数 | 占轻度用户比例 | 出行次数 | 人均出行 |
|-------------|--------|---------------|----------|----------|
"""

    # 统计2-5次和6-9次用户
    light_2_5 = {uid: s for uid, s in light_users.items() if s['total_trips'] <= 5}
    light_6_9 = {uid: s for uid, s in light_users.items() if s['total_trips'] >= 6}

    light_2_5_count = len(light_2_5)
    light_2_5_trips = sum(s['total_trips'] for s in light_2_5.values())
    light_6_9_count = len(light_6_9)
    light_6_9_trips = sum(s['total_trips'] for s in light_6_9.values())

    md_content += f"| 2-5次出行 | {light_2_5_count:,} | {light_2_5_count/light_count*100:.1f}% | {light_2_5_trips:,} | {light_2_5_trips/light_2_5_count:.1f} |\n"
    md_content += f"| 6-9次出行 | {light_6_9_count:,} | {light_6_9_count/light_count*100:.1f}% | {light_6_9_trips:,} | {light_6_9_trips/light_6_9_count:.1f} |\n"

    md_content += f"""
**关键发现**：
- 轻度用户占总用户的 {light_count/total_users*100:.1f}%，是最大的用户群体
- 2-5次用户占轻度用户的 {light_2_5_count/light_count*100:.1f}%，是激活的重点目标
- 6-9次用户（{light_6_9_count:,}人）最接近中度用户门槛，是转化的关键群体

#### 7.4.2 卡类型分布

| 卡类型 | 用户数 | 占轻度用户比例 | 出行次数 | 客流占比 | 人均出行 |
|--------|--------|---------------|----------|----------|----------|
"""

    # 卡类型分布
    light_card_dist = Counter(s['card_type'] for s in light_users.values())
    light_card_trips = Counter()
    for s in light_users.values():
        light_card_trips[s['card_type']] += s['total_trips']

    for card_type, count in light_card_dist.most_common():
        user_pct = count / light_count * 100
        trips = light_card_trips[card_type]
        trip_pct = trips / light_trips * 100
        avg_trips = trips / count
        md_content += f"| {card_type} | {count:,} | {user_pct:.1f}% | {trips:,} | {trip_pct:.1f}% | {avg_trips:.1f} |\n"

    # ==================== 7.4.3 付费情况 ====================
    light_paid = sum(1 for s in light_users.values() if s['paid_trip_count'] > 0)
    light_free = light_count - light_paid
    light_revenue = sum(s['total_paid_amount'] for s in light_users.values())

    md_content += f"""
#### 7.4.3 付费情况

* **付费用户**: {light_paid:,} ({light_paid/light_count*100:.1f}%)
* **免费用户**: {light_free:,} ({light_free/light_count*100:.1f}%)
* **总营收**: ¥{light_revenue:,.2f}
* **人均付费**: ¥{light_revenue/light_paid:.2f} (仅付费用户)

**付费对比**：
- 轻度用户付费率 ({light_paid/light_count*100:.1f}%) vs 重度用户付费率 (29.4%)
- 轻度用户人均付费 (¥{light_revenue/light_paid:.2f}) vs 重度用户人均付费 (¥136.75)
...
"""
    ...
    MOBILE_PAYMENT_TYPES = {'支付宝离线', '微信同程乘车码', '云闪二维码', '银联乘车码', '银行云闪'}
    light_mobile = {uid: s for uid, s in light_users.items() if s['card_type'] in MOBILE_PAYMENT_TYPES}
    light_mobile_count = len(light_mobile)

    light_id = {uid: s for uid, s in light_users.items() if s['card_type'] == '身份证'}
    light_id_count = len(light_id)

    md_content += f"| 移动支付用户 | {light_mobile_count:,} | {light_mobile_count/light_count*100:.1f}% | 支付宝/微信/云闪付等 |\n"
    md_content += f"| 身份证用户 | {light_id_count:,} | {light_id_count/light_count*100:.1f}% | 老年卡（免费） |\n"
    md_content += f"| 其他卡类型 | {light_count - light_mobile_count - light_id_count:,} | {(light_count - light_mobile_count - light_id_count)/light_count*100:.1f}% | 学生卡、爱心卡等 |\n"

    md_content += f"""
**关键发现**：
- 轻度用户中移动支付用户占比 {light_mobile_count/light_count*100:.1f}%，远高于重度用户（19.5%）
- 移动支付用户更可能呈现轻度使用特征
- 激活移动支付轻度用户可显著提升营收

#### 7.4.5 线路使用特征

**平均每位轻度用户使用线路数**: {sum(len(s.get('route_distribution', {})) for s in light_users.values()) / light_count:.1f}条
...
"""
    ...
    light_route_count = Counter()
    for uid, s in light_users.items():
        for route, trips in s.get('route_distribution', {}).items():
            light_route_count[route] += trips

    for rank, (route, count) in enumerate(light_route_count.most_common(10), 1):
        pct = count / light_trips * 100
        md_content += f"| {rank} | {route}路 | {count:,} | {pct:.1f}% |\n"
...
| 当前群体 | 用户数 | 转化目标 | 策略建议 |
|---------|--------|---------|----------|
| 2-5次用户 | {light_2_5_count:,} | 提升至6-9次 | 推送优惠套餐、定期乘车奖励 |
| 6-9次用户 | {light_6_9_count:,} | 提升至10+次（中度用户） | 月票推广、通勤优惠、会员权益 |

**激活策略建议**：
- **针对移动支付轻度用户**：推送电子钱包优惠券、乘车折扣券
- **针对2-5次用户**：设置"乘车N次送1次"的激励计划
- **针对6-9次用户**：推荐月票或次卡，强调经济性（当前人均{light_trips/light_count:.1f}次）
- **价格敏感度分析**：轻度用户付费率{light_paid/light_count*100:.1f}%，可通过优惠活动提升

#### 7.4.7 关键发现与建议

**轻度用户行为特征总结**：
- **用户规模**：轻度用户{light_count:,}人，占总用户的{light_count/total_users*100:.1f}%
- **移动支付占比高**：{light_mobile_count/light_count*100:.1f}%的轻度用户使用移动支付，远高于重度用户
- **线路集中度低**：平均使用{sum(len(s.get('route_distribution', {})) for s in light_users.values()) / light_count:.1f}条线路，出行分散
- **付费率中等**：{light_paid/light_count*100:.1f}%的轻度用户有付费行为
- **人均出行{light_trips/light_count:.1f}次**：存在较大激活空间

**运营建议**：
- **优先激活6-9次用户**：{light_6_9_count:,}位用户最接近中度用户门槛，通过月票推广可快速转化
- **移动支付精准营销**：针对{light_mobile_count:,}位移动支付轻度用户推送电子优惠券
- **设置成长激励**：建立用户等级体系，从轻度用户逐步培养至重度用户
- **潜力评估**：若能将10%的轻度用户转化为中度用户，可增加约{int(light_count * 0.1 * (10 - light_trips/light_count)):,}次出行
...
"""
    ...
    # ==================== 移动支付用户专项分析 ====================
    md_content += generate_mobile_payment_analysis(user_stats)
